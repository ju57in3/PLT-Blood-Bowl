...
namespace ai {
    ...
    bool HeuristicAI::runAI() {
        ...
        // Priorité 2: Aller vers la balle avec le personnage le plus proche
        auto ballPos = game->getBallPosition();
        std::shared_ptr<state::Character> closest = nullptr;
        int bestDist = std::numeric_limits<int>::max();

        // Trouver le personnage le plus proche de la balle
        for (const auto& c : myTeam->getCharacters()) {
            if (!c || c->getStatus() != state::playable) continue;
        ...
        // Si on a trouvé un personnage, le déplacer vers la balle
        if (closest) {
        ...
        // Priorité 3: Fallback - déplacer le premier personnage jouable aléatoirement
        for (const auto& c : myTeam->getCharacters()) {
            if (!c || c->getStatus() != state::playable) continue;
        ...
        std::cout << "[HEURISTIC AI] No playable characters found\n";
        return false;
    }
}
