...
Contrairement à l’IA aléatoire, l’IA heuristique repose sur un ensemble de règles simples visant à produire un comportement cohérent et orienté vers l’objectif principal du jeu : contrôler le ballon et marquer un touchdown, ou empêcher l’adversaire de le faire.

Cette IA ne réalise pas de recherche exhaustive ni de planification à long terme. Elle s’appuie sur des heuristiques locales, c’est-à-dire des critères simples évalués à l’instant présent, pour choisir quelle action effectuer et avec quel joueur.

Les principes généraux de la stratégie sont les suivants :

- Prioriser les actions à fort impact immédiat, comme le blocage du porteur de balle adverse.
- Exploiter le porteur de balle allié lorsqu’il existe, en le faisant progresser vers la zone d’en-but adverse.
- À défaut de porteur de balle, déplacer les joueurs disponibles vers le ballon ou vers le porteur adverse.
- Ne jamais effectuer d’actions illégales : seuls les joueurs debout et jouables sont considérés.

Chaque tour est ainsi une succession de décisions locales, guidées par la position du ballon, des porteurs de balle et des distances entre joueurs.
...
`HeuristicAI` est une extension directe de la classe `AI`, tout comme `RandomAI`.  
Elle redéfinit la méthode `runAI()` mais introduit une logique décisionnelle plus structurée, reposant sur :

- des fonctions utilitaires locales (distance, sélection de joueurs, vérifications),
- une boucle de décision bornée en nombre d’actions,
- un ordre de priorité clair entre les différentes heuristiques.

Les fonctions utilitaires sont définies dans un namespace anonyme, ce qui limite leur portée au fichier source et évite toute pollution de l’interface publique.

#### 5.3.2 Fonctions heuristiques et utilitaires

Plusieurs fonctions statiques servent de briques de base à la prise de décision :

- **Distance de Chebyshev**  
  Utilisée pour approximer la distance entre deux positions sur la grille du terrain, adaptée aux déplacements autorisant les diagonales.

- **Filtrage des joueurs jouables**  
  Un joueur est considéré pour une action uniquement s’il est :
    - non nul,
    - debout,
    - dans l’état `playable`.

- **Recherche du porteur de balle**  
  Une fonction parcourt l’équipe pour identifier le joueur possédant le ballon, s’il existe.

- **Sélection du joueur le plus proche d’une cible**  
  Permet de désigner le meilleur candidat pour se rapprocher du ballon ou du porteur adverse.

- **Calcul du meilleur déplacement**  
  Pour un joueur donné, toutes les positions atteignables dans la limite de son mouvement sont évaluées, et celle qui minimise la distance à la cible est choisie, sous réserve qu’elle soit dans le terrain et non occupée.

Ces fonctions forment la base de l’IA composée : elles peuvent être réutilisées ou enrichies dans des versions plus avancées.

#### 5.3.3 Vérifications de cohérence du tour

Au début de `runAI()`, l’IA vérifie plusieurs conditions bloquantes :

- l’état du jeu est valide,
- c’est bien le tour de l’équipe contrôlée,
- l’état courant est un `PlayerTurn`,
- le tour n’est pas déjà terminé,
- aucun touchdown n’a déjà été marqué.

Ces garde-fous évitent toute exécution incohérente et rendent l’IA robuste face aux transitions d’état du moteur.

#### 5.3.4 Boucle de décision et limite d’actions

L’IA fonctionne dans une boucle bornée par un nombre maximal d’actions par tour (11).  
À chaque itération, elle réévalue complètement la situation du jeu, ce qui permet de réagir immédiatement aux conséquences des actions précédentes (blocage réussi, chute, turnover, etc.).

La boucle s’arrête dès que :
- le tour change,
- un turnover survient,
- un touchdown est marqué,
- aucun joueur jouable n’est disponible,
- aucune action pertinente n’est trouvée.

#### 5.3.5 Ordre de priorité des heuristiques

À chaque itération, les actions sont évaluées dans l’ordre suivant :

1. **Blocage du porteur de balle adverse**  
   Si un porteur de balle adverse existe et qu’un joueur allié jouable est adjacent, un blocage est immédiatement déclenché.  
   Cette règle reflète une priorité défensive forte.

2. **Avancée du porteur de balle allié**  
   Si l’IA possède le ballon, le porteur est déplacé vers la ligne de touchdown adverse, déterminée dynamiquement selon l’équipe contrôlée.

3. **Blocage opportuniste**  
   Si un joueur sélectionné est adjacent à un adversaire debout, un blocage est tenté, même si l’adversaire ne porte pas le ballon.

4. **Déplacement vers un objectif**  
   À défaut de blocage, le joueur le plus proche du ballon (ou du porteur adverse) est déplacé dans la direction de cette cible.

Cet ordre garantit que les actions les plus critiques sont tentées en premier.

#### 5.3.6 Interaction avec le moteur

Comme pour les autres IA, `HeuristicAI` n’agit jamais directement sur l’état du jeu.  
Toutes les actions passent par les commandes du moteur :

- `Move` pour les déplacements,
- `Block` pour les blocages.

Chaque commande est immédiatement exécutée, ce qui permet à l’IA de recalculer ses décisions à partir d’un état de jeu mis à jour.

#### 5.3.7 Fin de tour

Une fois la boucle terminée, l’IA force la fin du tour en positionnant l’état `turnOver`.  
La valeur de retour de `runAI()` indique si au moins une action a été effectuée durant le tour.

Cette architecture rend l’IA facilement extensible : de nouvelles heuristiques peuvent être ajoutées en amont de la boucle ou insérées dans l’ordre de priorité existant, sans remettre en cause l’ensemble de la conception.
